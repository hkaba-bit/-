...
### [2026-09-22] Jev（TypeSafe AI）の調査と活用先の洗い出し — Claude
- 成果物: `references/jev-research.md`
- 検証: 社内（gg-manager の案件・議事録・チャット、Notion）を検索したが **「JEV」の記録なし**。社外の新規プロダクトとして一次情報と国内解説2本を読んだ
- 判断メモ:
  - Jev は 2026-09-15 発表の「System One Model」。文章を生成せず、**型付きの判断＋確率**を返す（Choice / Score / Noul の3種）。応答 70〜500ms、入力 $0.042/100万トークン、出力無料
  - **精度は上がらない。**判断タスクの一致率は Jev 67.8% 対 GPT-5.6 Terra 67.9% でほぼ同等。価値は速さと安さだけ
  - 「ハルシネーションしない」は**型を外れないという意味**で、判断の誤りは残る。ここを取り違えると事故る
  - うちで効くのは「同じ判断を数百〜数千回繰り返す」場面に限られる。**記事移管のカテゴリ判定（城北埼玉で2,365件）**と**更新性◎○△×の判定**が筆頭
  - 提案書の原稿・ワイヤーのコピー・調査レポートは**文章を書けないので対象外**
  - 発表から1週間・Early Access・性能値は自社評価という段階なので、急いで入れる理由はないと判断した
- 残課題: 次のリニューアル案件で用途B（更新性判定）を小さく試す。過去案件の仕様書が正解データになる。送信データが `AGENTS.md` 第3章に触れないかを先に判断する

### [2026-09-18] 城北埼玉：要件定義の一式を並列セッションで着手 — Claude
- 成果物: `projects/johoku-saitama/`（`research/brief.md` / `research/current-site-audit.md` / `STATUS.md`）。下流3本は並列セッションへ委譲
- 検証: 現行サイトを実測（クロール142ページ、HTTPヘッダ、トップのHTMLソース95,161文字）
- 判断メモ:
  - 依頼は「案件作成から提案まで一式」だったが、**この案件は昨日（9/17）受注済み**で提案書は提出・承認済みだった。新規に提案書を作るのは重複になるため、Backlog のネクストアクション「要件定義（サイトマップ・見積精緻化）」の一式に読み替えた
  - 実測で P0 を検出：コメントアウトした旧ナビの内部に出力済みの `-->` が残り、HTMLコメントが途中で閉じている。削除したはずのリンクが実リンクとして出力され、不正URLが約90本（到達URLの約47%）生きている。仕様書が言う「URL体系の不統一・旧ページ残存」の実体
  - 既存仕様書は「戦略・戦術シート」「デザインコンセプト」が全て空欄、「数値目標」はサンプル値のまま。**ここを埋めるのが今回の仕事**と位置づけた
  - 与件整理と現行サイト調査を先に単独で書き、それを土台に3セッションへ分割した。会話履歴は渡さず、Goal / Context / Constraints / Done-when の4点のみ（`references/role-split.md` の引き渡しの型）
- 残課題: 3本の PR のレビューと統合。GA4 実数の取得（未取得のまま数値目標を出せない）
...
| 案件スラッグ | 状況 | 次アクション | 期日 |
|---|---|---|---|
| `johoku-saitama` | 受注済み（2026-09-17・400万円）。要件定義の一式（仕様書Excel・ワイヤー6本・要件定義報告PPTX 82枚）を作成し、**検品のうえ統合済み** | ①GA4 実数を先方に依頼 ②受注プラン（梅/竹/松）確定＝16Pの内訳 ③Codex レビュー ④PDF→画像の目視確認（ローカル） | 10月中に戦略設計・見積精緻化 |
